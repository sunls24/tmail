...
func (_q *AttachmentQuery) QueryOwner() *EnvelopeQuery {
	query := (&EnvelopeClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *AttachmentQuery) FirstIDX(ctx context.Context) string {
...
func (_q *AttachmentQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *AttachmentQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *AttachmentQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *AttachmentQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *AttachmentQuery) WithOwner(opts ...func(*EnvelopeQuery)) *AttachmentQuery {
	query := (&EnvelopeClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Attachment, e *Envelope) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AttachmentQuery) loadOwner(ctx context.Context, query *EnvelopeQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Envelope)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Attachment)
	for i := range nodes {
		if nodes[i].envelope_attachments == nil {
			continue
		}
		fk := *nodes[i].envelope_attachments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "envelope_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
